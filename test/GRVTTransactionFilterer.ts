...
describe("GRVTTransactionFilterer", function () {
  ...
  describe("Deployment", function () {
    it("Should set the right initial values", async function () {
      const { grvtTransactionFilterer, owner, l1SharedBridgeAddress, l2BridgeAddress } =
  ...
  describe("Transaction filtering", function () {
    ...
    it("Should reject transaction with l1 sender != grvtBridgeProxyAddress", async function () {
      const { grvtTransactionFilterer, rando, l1SharedBridgeAddress, l2BridgeAddress } =
        await deployGRVTTransactionFiltererFixture()
      expect(
        await grvtTransactionFilterer.isTransactionAllowed(
          l1SharedBridgeAddress,
    ...
    it("Should reject transaction with tx sender != l1SharedBridgeAddress", async function () {
      const { grvtTransactionFilterer, rando, l2BridgeAddress, grvtBridgeProxyAddress } =
        await deployGRVTTransactionFiltererFixture()
      expect(
        await grvtTransactionFilterer.isTransactionAllowed(
          rando.address,
    ...
    it("Should reject transaction with l2Contract != l2BridgeAddress", async function () {
      const { grvtTransactionFilterer, rando, l1SharedBridgeAddress, grvtBridgeProxyAddress } =
        await deployGRVTTransactionFiltererFixture()
      expect(
        await grvtTransactionFilterer.isTransactionAllowed(
          l1SharedBridgeAddress,
    ...
    it("Should reject transaction with invalid calldata", async function () {
      ...
      await expect(
        grvtTransactionFilterer.isTransactionAllowed(
          l1SharedBridgeAddress,
          l2BridgeAddress,
          0,
          0,
...
async function getDepositL2Calldata(
  ...
  const calldata = IL2BridgeInterface.encodeFunctionData(
    ...
    "finalizeDeposit",
    [l1Sender, l2Receiver, l1Token, amount, "0x"]
  )
